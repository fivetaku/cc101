...
<strong>Sub-agents(서브에이전트)</strong>는 메인 Claude가 특정 작업을 전문화된 별도의 Claude 인스턴스에 위임하는 기능입니다.

> **쉬운 비유**: 팀장이 프로젝트를 받으면 혼자 다 하지 않고 팀원에게 나눠 맡깁니다. "리서치는 A에게, 번역은 B에게, 최종 교정은 C에게" — Sub-agents는 Claude가 이렇게 작업을 분담하는 방식입니다.

각 서브에이전트는:
- <strong>독립된 컨텍스트 창</strong>을 가집니다 (메인 대화와 분리)
- <strong>커스텀 시스템 프롬프트</strong>로 특정 역할에 집중합니다
- <strong>제한된 도구</strong>만 사용합니다 (필요한 것만)
