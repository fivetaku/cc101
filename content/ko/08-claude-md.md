...
여러 CLAUDE.md 파일이 있을 때는 내용이 <strong>모두 함께 로드</strong>되고, 겹치는 지시는 더 구체적인 파일 쪽이 우선하는 경향이 있습니다.

```
개인 설정 간 우선 경향 (구체적 → 일반적):
1. CLAUDE.local.md (나만의 프로젝트 설정)
2. ./CLAUDE.md (프로젝트 설정)
3. ~/.claude/CLAUDE.md (전역 설정)

※ 조직 관리(managed) CLAUDE.md는 예외 — 회사 IT가 배포하는 파일로,
   개인 설정으로 끌 수 없으며 항상 함께 강제 적용됩니다.
```
