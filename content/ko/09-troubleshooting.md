...
먼저 `claude doctor`를 실행해서 문제를 진단해보세요.
...
```bash
# ⚠️ 위 방법으로 해결되지 않을 때만 사용하세요
rm ~/.claude.json
rm -rf ~/.claude/
claude
```

> ⚠️ **주의**: 이 명령은 모든 설정, 세션 기록, 플러그인 설정을 삭제합니다. 반드시 `/doctor`를 먼저 시도하세요.
...
`Shift+Tab`으로 Plan Mode 진입 → Claude가 "어디를 어떻게 바꿀 예정"인지 먼저 보여줍니다. 확인 후 승인.
...
```
Claude Code 시작 전:
□ 프로젝트 폴더로 이동했나? (cd ~/Documents/my-project)
