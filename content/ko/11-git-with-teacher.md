...
> 🖥️ 이 섹션은 주로 <strong>코드를 작성하는 개발자</strong>를 위한 내용입니다. 코딩 작업이 없다면 아래 **비개발자 파일 버전 관리** 팁만 읽고 다음 섹션으로 넘어가도 됩니다. 명령어를 몰라도 괜찮습니다 — 자연어로 시작하면 됩니다.
...
| 상황 | 방법 |
|------|------|
| **문서 버전 기록** | Google Docs·Notion의 "버전 기록" 기능 — 언제든 이전 버전으로 복구 가능 |
| **작업 전 스냅샷** | `"reports/ 폴더를 reports_backup_오늘날짜로 복사해줘"` (Claude Code에 말하면 됩니다) |
| **작업 되돌리기** | `Esc Esc`(입력창 빈 상태) 또는 `/rewind` — 되감기 메뉴가 열리고, 시점을 골라 복원합니다 |

> ⚠️ **되감기의 한계**: 되감기는 Claude가 <strong>파일 편집 도구로 직접 고친 것</strong>만 되돌릴 수 있습니다. 터미널 명령(bash)으로 복사·이동·삭제된 파일은 되감기로 복구되지 않습니다 — 위의 "폴더 복사 스냅샷"이 바로 그런 경우라, 복사본 자체가 백업 역할을 합니다.
...
슬래시 명령어로도 사용 가능합니다 (전용 커맨드가 하나씩 있습니다):

| 자연어 | 슬래시 |
|-------|--------|
| "깃 시작해줘" (첫 설정) | `/git-teacher-setup` |
| "지금 어떤 상태?" | `/git-teacher-status` |
| "저장해줘" | `/git-teacher-save` |
| "올려줘" | `/git-teacher-upload` |
| "검토 요청해줘" | `/git-teacher-review` |
| 용어 질문 | `/git-teacher-help` |
